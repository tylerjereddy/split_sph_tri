...
def determine_Cartesian_coords_D(input_coords):
    '''Determine the Cartesian coords of the desired center
    point D that splits the spherical triangle into three
    equal area subtriangles.'''
    # calculate the central angle of each arc
    central_angle_AD = calc_central_angle(arc_length_AD, sphere_radius)
    central_angle_BD = calc_central_angle(arc_length_BD, sphere_radius)
    central_angle_CD = calc_central_angle(arc_length_CD, sphere_radius)
    # calculate the chord lengths for each arc
    chord_length_AD = calc_chord_length(sphere_radius, central_angle_AD)
    chord_length_BD = calc_chord_length(sphere_radius, central_angle_BD)
    chord_length_CD = calc_chord_length(sphere_radius, central_angle_CD)
    # use Pythagorean sum of squares to establish a set of three
    # simultaneous equations with three unknowns (for delta x, y, z
    # values relative to one of the known spherical triangle
    # vertices)
    
    # need Cartesian coords of input spherical triangle vertices
    A = input_coords[0, ...]
    B = input_coords[1, ...]
    C = input_coords[2, ...]
    # use delta_x_3 & so on for Cartesian deltas from point C
    # to point D
    # define some constants for concision of simultaneous equations
    K1 = C[0] - A[0]
    K2 = C[1] - A[1]
    K3 = C[2] - A[2]
    K4 = C[0] - B[0]
    K5 = C[1] - B[1]
    K6 = C[2] - B[2]
    # the only unknowns here are the delta_ values
    # everything else is a known constant
    chord_length_AD ** 2 = K1 ** 2 + (2 * K1 * delta_x_3) + delta_x_3 ** 2 + \
                           K2 ** 2 + (2 * K2 * delta_y_3) + delta_y_3 ** 2 + \
                           K3 ** 2 + (2 * K3 * delta_z_3) + delta_z_3 ** 2

    chord_length_BD ** 2 = K4 ** 2 + (2 * K4 * delta_x_3) + delta_x_3 ** 2 + \
                           K5 ** 2 + (2 * K5 * delta_y_3) + delta_y_3 ** 2 + \
                           K6 ** 2 + (2 * K6 * delta_z_3) + delta_z_3 ** 2

    chord_length_CD ** 2 = delta_x_3 ** 2 + delta_y_3 ** 2 + delta_z_3 ** 2
    # there are three equations with three unknowns, but they are
    # quadratic simultaneous equations, which may mean a bit of a mess?





def calc_chord_length(sphere_radius, central_angle):
    # see http://mathworld.wolfram.com/CircularSegment.html
    return 2. * sphere_radius * math.sin(0.5 * central_angle)

def calc_central_angle(arc_length, sphere_radius):
    # see http://mathworld.wolfram.com/CircularSegment.html
    return arc_length / sphere_radius
